    ...
    <>
      <div className="Navbar">
        <div className="container">
          <div className="menuIcon" onClick={() => setOpen(true)}>☰</div>
          ...
          <div className="logoname">VELOUR</div>
            ...
            {session ? (
              <div className="user-menu" ref={dropdownRef}>
                ...
                <div className={`user-dropdown ${dropdownOpen ? "show" : ""}`}>
                  <Link href="/Myorders" onClick={() => setDropdownOpen(false)}>My Orders</Link>
                  <button onClick={() => signOut()}>Log Out</button>
                </div>
              </div>
            ) : (
              <Link href="/Auth/login" className="login">Sign in</Link>
            )}
          </div>
        </div>
      </div>
      ...
      <div className={`sidebar ${open ? "active" : ""}`}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", flexDirection: "column", height: "100%" }}>
          ...
          <div>
            {session ? (
              <button className="logout" onClick={() => { signOut(); setOpen(false); }}>Log Out</button>
            ) : (
              <Link href="/Auth/login" className="login" onClick={() => setOpen(false)}>Sign In</Link>
            )}
          </div>
        </div>
      </div>
